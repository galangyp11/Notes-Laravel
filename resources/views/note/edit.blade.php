<x-layout>
    <div class="bg-gray-100 grid grid-rows-6 grid-flow-col rounded-md p-4 h-[41em] lg:w-1/2 mx-auto">
        ...
        <form action="{{ route("note.update", $note)}}" method="POST" class="w-full row-span-5 grid grid-rows-6 grid-flow-col h-full ">
            @csrf
            @method('PUT')
            <textarea name="note" class="bg-white p-4 row-span-5 rounded-md w-full h-full resize-none" id="" cols="30"  rows="10" placeholder="Type Note Here... ">{{$note->note}}</textarea>
